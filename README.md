...
- 音声ファイルは Windows のユーザディレクトリ配下 `Music/beep-copilot.wav` を直接参照します。

通知音を変える場合は Windows 側の `Music/beep-copilot.wav` を差し替えてください。PowerShell の `SoundPlayer` を使うため、音声ファイルは `.wav` を前提にしています。
